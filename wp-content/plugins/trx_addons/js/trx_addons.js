(function () {
    ...
    window.trx_addons_get_cookie = function (name) {
        var defa = arguments[1] !== undefined
            ? arguments[1]
            : null;
        var start = document
            .cookie
    ...
    window.trx_addons_set_cookie = function (name, value) {
        var expires = arguments[2] !== undefined
            ? arguments[2]
            : 0;
        var path = arguments[3] !== undefined
            ? arguments[3]
            : '/';
        var domain = arguments[4] !== undefined
            ? arguments[4]
            : '';
        var secure = arguments[5] !== undefined
            ? arguments[5]
            : '';
        var today = new Date();
    ...
    window.trx_addons_del_cookie = function (name) {
        var path = arguments[1] !== undefined
            ? arguments[1]
            : '/';
        var domain = arguments[2] !== undefined
            ? arguments[2]
            : '';
...
window.trx_addons_form_validate = function (form, opt) {
                    ...
                    if (jQuery(this).attr("name") == opt.rules[i].field) {
                        ...
                        if (typeof(opt.rules[i].min_length) == 'object') {
                            if (opt.rules[i].min_length.value > 0 && val.length < opt.rules[i].min_length.value) {
                                ...
                                error_msg += '<p class="trx_addons_error_item">' + (typeof(opt.rules[i].min_length.message) != 'undefined'
                                    ? opt.rules[i].min_length.message
                                    : opt.error_message_text) + '</p>';
                                error = true;
                            }
                        }
                        if ((!error || !opt.exit_after_first_error) && typeof(opt.rules[i].max_length) == 'object') {
                            if (opt.rules[i].max_length.value > 0 && val.length > opt.rules[i].max_length.value) {
                                if (error_msg === '') 
                                    jQuery(this).get(0).focus();
                                error_msg += '<p class="trx_addons_error_item">' + (typeof(opt.rules[i].max_length.message) != 'undefined'
                        ...
                        if ((!error || !opt.exit_after_first_error) && typeof(opt.rules[i].mask) == 'object') {
                            if (opt.rules[i].mask.value !== '') {
                                ...
                                if (!regexp.test(val)) {
                                    if (error_msg === '') 
                                        jQuery(this).get(0).focus();
                                    error_msg += '<p class="trx_addons_error_item">' + (typeof(opt.rules[i].mask.message) != 'undefined'
                        ...
                        if ((!error || !opt.exit_after_first_error) && typeof(opt.rules[i].state) == 'object') {
                            if (opt.rules[i].state.value == 'checked' && !jQuery(this).get(0).checked) {
                                if (error_msg === '') 
                                    jQuery(this).get(0).focus();
                                error_msg += '<p class="trx_addons_error_item">' + (typeof(opt.rules[i].state.message) != 'undefined'
                        ...
                        if ((!error || !opt.exit_after_first_error) && typeof(opt.rules[i].equal_to) == 'object') {
                            if (opt.rules[i].equal_to.value !== '' && val != jQuery(jQuery(this).get(0).form[opt.rules[i].equal_to.value]).val()) {
                                if (error_msg === '') 
                                    jQuery(this).get(0).focus();
                                error_msg += '<p class="trx_addons_error_item">' + (typeof(opt.rules[i].equal_to.message) != 'undefined'
    ...
    window.trx_addons_refresh_list = function (parent_type, parent_val, list_fld, list_lbl, list_not_selected) {
        if (list_not_selected === undefined) {
            var list_not_selected = list_fld.data('not-selected') === true || list_fld
                .parents('.vc_edit-form-tab')
                .length > 0 || list_fld
                .parents('#elementor-controls')
                .length > 0 || list_fld
                .parents('.siteorigin-widget-field')
                .length > 0 || list_fld
                .parents('[class*="widget_field_type_"]')
                .length > 0 || list_fld
                .parents('.widget-liquid-right')
                .length > 0 || list_fld
                .parents('.widgets-holder-wrap')
                .length > 0 || list_fld
                .parents('.customize-control-widget_form')
                .length > 0;
        }
        ...
        jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], data, function (response) {
            ...
            if (rez.error === '') {
                ...
                for (var i in rez.data) {
                    if (list_type != 'select' && rez.data[i]['key'] == 0) 
                        continue;
                    opt_list += list_type == 'select'
                        ? '<option class="' + rez.data[i]['key'] + '" value="' + rez.data[i]['key'] + '"' + (rez.data[i]['key'] == list_val
                            ? ' selected="selected"'
                            : '') + '>' + rez.data[i]['value'] + '</option>'
                        : '<label><input type="checkbox" value="' + rez.data[i]['key'] + '" name="' + list_name + '">' + rez.data[i]['value'] + '</label>';
                }
    ...
    window.trx_addons_fixed_rows_height = function () {
        var with_admin_bar = arguments.length > 0
            ? arguments[0]
            : true;
        var with_fixed_rows = arguments.length > 1
            ? arguments[1]
            : true;
        var oft = 0;
        if (with_admin_bar) {
        ...
        if (with_fixed_rows) {
            jQuery('.sc_layouts_row_fixed_on')
                .each(function () {
    ...
    window.trx_addons_add_to_url = function (loc, prm) {
        var ignore_empty = arguments[2] !== undefined
            ? arguments[2]
            : true;
        var q = loc.indexOf('?');
        var attr = {};
    ...
    window.trx_addons_set_autoplay = function (container, value) {
        if (value === undefined) 
            value = 1;
        container
            .find('.video_frame > iframe, iframe')
            .each(function () {
    ...
    window.trx_addons_browser_is_ios = function () {
        return navigator
            .userAgent
            .match(/iPad|iPhone|iPod/i) != null || navigator
            .platform
    ...
    window.trx_addons_is_retina = function () {
        var mediaQuery = '(-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-m' +
                'in-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx)';
    ...
    window.trx_addons_round_number = function (num) {
        var precision = arguments[1] !== undefined
            ? arguments[1]
            : 0;
    ...
    window.trx_addons_parse_codes = function (text, tag_start, tag_end) {
        if (tag_start === undefined) 
            tag_start = '{{';
        if (tag_end === undefined) 
            tag_end = '}}';
    ...
    window.trx_addons_rgb2hex = function (color) {
        ...
        if (aRGB) {
            color = '';
            for (var i = 1; i <= 3; i++) 
                color += Math.round((aRGB[i][aRGB[i].length - 1] == "%"
                    ? 2.55
                    : 1) * parseInt(aRGB[i])).toString(16).replace(/^(.)$/, '0$1');
            }
        else 
    ...
    window.trx_addons_components2hex = function (r, g, b) {
        return '#' + Number(r)
            .toString(16)
            .toUpperCase()
            .replace(/^(.)$/, '0$1') + Number(g)
            .toString(16)
            .toUpperCase()
            .replace(/^(.)$/, '0$1') + Number(b)
            .toString(16)
            .toUpperCase()
            .replace(/^(.)$/, '0$1');
    };
    ...
    window.trx_addons_hex2hsb = function (hex) {
        var h = arguments[1] !== undefined
            ? arguments[1]
            : 0;
        var s = arguments[2] !== undefined
            ? arguments[2]
            : 0;
        var b = arguments[3] !== undefined
            ? arguments[3]
            : 0;
    ...
    window.trx_addons_rgb2hsb = function (rgb) {
        ...
        hsb.s = (hsb.b <= 0)
            ? 0
            : Math.round(100 * (hsb.b - Math.min(Math.min(rgb.r, rgb.g), rgb.b)) / hsb.b);
    ...
    window.trx_addons_color_picker = function () {
        ...
        var colors = arguments[1] !== undefined
            ? arguments[1]
            : '#f00,#ff0,#0f0,#0ff,#00f,#f0f,#fff,#ebebeb,#e1e1e1,#d7d7d7,#cccccc,#c2c2c2,#b7b7' +
                'b7,#acacac,#a0a0a0,#959595,#ee1d24,#fff100,#00a650,#00aeef,#2f3192,#ed008c,#8989' +
                    '89,#7d7d7d,#707070,#626262,#555,#464646,#363636,#262626,#111,#000,#f7977a,#fbad8' +
                    '2,#fdc68c,#fff799,#c6df9c,#a4d49d,#81ca9d,#7bcdc9,#6ccff7,#7ca6d8,#8293ca,#8881b' +
                    'e,#a286bd,#bc8cbf,#f49bc1,#f5999d,#f16c4d,#f68e54,#fbaf5a,#fff467,#acd372,#7dc47' +
                    '3,#39b778,#16bcb4,#00bff3,#438ccb,#5573b7,#5e5ca7,#855fa8,#a763a9,#ef6ea8,#f16d7' +
                    'e,#ee1d24,#f16522,#f7941d,#fff100,#8fc63d,#37b44a,#00a650,#00a99e,#00aeef,#0072b' +
                    'c,#0054a5,#2f3192,#652c91,#91278f,#ed008c,#ee105a,#9d0a0f,#a1410d,#a36209,#aba00' +
                    '0,#588528,#197b30,#007236,#00736a,#0076a4,#004a80,#003370,#1d1363,#450e61,#62055' +
                    'f,#9e005c,#9d0039,#790000,#7b3000,#7c4900,#827a00,#3e6617,#045f20,#005824,#00595' +
                    '1,#005b7e,#003562,#002056,#0c004b,#30004a,#4b0048,#7a0045,#7a0026';
        ...
        tbl += '</tr></thead><tbody><tr style="height:60px;"><td colspan="8" id="' + id + '_colorPreview" style="vertical-align:middle;text-align:center;border:1px solid #' +
                '000;background:#fff;"><input style="width:55px;color:#000;border:1px solid rgb(0' +
                ', 0, 0);padding:5px;background-color:#fff;font:11px Arial, Helvetica, sans-serif' +
                ';" maxlength="7" /><a href="#" id="' + id + '_moreColors" class="iColorPicker_moreColors"></a></td><td colspan="8" id="' + id + '_colorOriginal" style="vertical-align:middle;text-align:center;border:1px solid ' +
                '#000;background:#fff;"><input style="width:55px;color:#000;border:1px solid rgb(' +
                '0, 0, 0);padding:5px;background-color:#fff;font:11px Arial, Helvetica, sans-seri' +
                'f;" readonly="readonly" /></td></tr></tbody></table>';
            ...
            .on('click', 'tbody .iColorPicker_moreColors', function (e) {
                ...
                if (thead.hasClass('more_colors')) {
                    ...
                    thead
                        ...
                        .empty()
                        .html(out + '<td style="background-color:#ffffff" colspan="8">&nbsp;</td></tr>');
    ...
    window.trx_addons_color_picker_show = function (id, fld, func) {
        ...
        var l = eICP.left + w < jQuery(window).width() - 10
            ? eICP.left
            : jQuery(window).width() - 10 - w;
        var t = eICP.top + fld.outerHeight() + h < jQuery(document).scrollTop() + jQuery(window).height() - 10
        ...
        switch (typeof(mixed_val)) {
            case "number":
                if (isNaN(mixed_val) || !isFinite(mixed_val)) 
                    ...
                    return (Math.floor(mixed_val) == mixed_val
                        ? "i"
                        : "d") + ":" + mixed_val + ";";
                case "string":
...
jQuery(document).ready(function () {
    ...
    function trx_addons_init_actions() {
        ...
        jQuery(document).on('action.switch_to_mobile_layout', function () {
            jQuery('[data-hover-animation^="animated"]')
                .each(function () {
    ...
    function trx_addons_ready_actions(e, container) {
        ...
        if (jQuery.ui && jQuery.ui.tabs && container.find('.trx_addons_tabs:not(.inited)').length > 0) {
            container
                .find('.trx_addons_tabs:not(.inited)')
                .each(function () {
        ...
        if (jQuery.ui && jQuery.ui.accordion && container.find('.trx_addons_accordion:not(.inited)').length > 0) {
            container
                .find('.trx_addons_accordion:not(.inited)')
                .each(function () {
        ...
        if (jQuery.ui && jQuery.ui.slider && container.find('.trx_addons_range_slider:not(.inited)').length > 0) {
            container
                .find('.trx_addons_range_slider:not(.inited)')
                .each(function () {
                    ...
                    var init_obj = {
                        ...
                        slide: function (event, ui) {
                            var cur_values = range_slider_type === 'min'
                                ? [ui.value]
                                : ui.values;
                            linked_field
                                .val(cur_values.join(','))
        ...
        if (container.find('.trx_addons_video_player.with_cover .video_hover:not(.inited)').length > 0) {
            container
                ...
                .addClass('inited')
                .on('click', function (e) {
                    if (jQuery(this).hasClass('trx_addons_popup_link')) 
                        return;
                    jQuery(this)
        ...
        if (TRX_ADDONS_STORAGE['popup_engine'] == 'pretty') {
            container
                .find("a[href$='jpg']:not(.inited),a[href$='jpeg']:not(.inited),a[href$='png']:not(.ini" +
                        "ted),a[href$='gif']:not(.inited)")
                .each(function () {
            ...
            var images = container
                .find("a[rel*='prettyPhoto']:not(.inited):not(.esgbox):not([data-rel*='pretty']):not([r" +
                    "el*='magnific']):not([data-rel*='magnific']):not([data-elementor-open-lightbox='" +
                    "yes']):not([data-elementor-open-lightbox='default'])")
                .addClass('inited');
            try {
            ...
            var images = container
                .find("a[rel*='magnific']:not(.inited):not(.esgbox):not(.prettyphoto):not([rel*='pretty" +
                    "']):not([data-rel*='pretty']):not([data-elementor-open-lightbox='yes']):not([dat" +
                    "a-elementor-open-lightbox='default'])")
                .addClass('inited');
            setTimeout(function () {
            ...
            container
                .find(".trx_addons_panel_link:not(.panel_inited)")
            ...
            window.trx_addons_prepare_popup_content = function (container, autoplay) {
                ...
                jQuery(window).trigger('resize');
                if (autoplay) 
                    trx_addons_set_autoplay(wrapper);
        ...
        if (container.find('a.post_counters_likes:not(.inited),a.comment_counters_likes:not(.inited)').length > 0) {
            ...
            container
                .find('a.post_counters_likes:not(.inited),a.comment_counters_likes:not(.inited)')
                .addClass('inited')
                .on('click', function (e) {
                    if (!likes_busy) {
                        likes_busy = true;
                        var button = jQuery(this);
                        var inc = button.hasClass('enabled')
                            ? 1
                            : -1;
                        var post_id = button.hasClass('post_counters_likes')
                            ? button.data('postid')
                            : button.data('commentid');
                        var cookie_likes = trx_addons_get_cookie(button.hasClass('post_counters_likes')
                            ? 'trx_addons_likes'
                            ...
                            .done(function (response) {
                                ...
                                if (rez.error === '') {
                                    ...
                                    if (inc == 1) {
                                        ...
                                        cookie_likes += (cookie_likes.substr(-1) != ','
                                            ? ','
                                            : '') + post_id + ',';
                                    } else {
                                    ...
                                    button
                                        .data('likes', counter)
                                        .attr('title', title)
                                        .find(button.hasClass('post_counters_likes')
                                            ? '.post_counters_number'
                                            : '.comment_counters_number')
                                        .html(counter);
                                    trx_addons_set_cookie(button.hasClass('post_counters_likes')
                                        ? 'trx_addons_likes'
                                        : 'trx_addons_comment_likes', cookie_likes, 365);
                                } else {
                ...
                .on('click', '.trx_addons_emotions_item', function (e) {
                    if (!emotions_busy) {
                        ...
                        jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], {
                            ...
                            emotion_dec: button_active.length > 0
                                ? button_active.data('slug')
                                : ''
                            })
                            .done(function (response) {
                                var rez = {};
                ...
                .each(function () {
                    jQuery(this)
                        ...
                        .on('click', '.social_item_popup', function (e) {
                            ...
                            window.open(url, '_blank', 'scrollbars=0, resizable=1, menubar=0, left=100, top=100, width=480, height=400, ' +
                                    'toolbar=0, status=0');
                            e.preventDefault();
                            return false;
                        });
        ...
        if (TRX_ADDONS_STORAGE['menu_cache'] && TRX_ADDONS_STORAGE['menu_cache'].length > 0) {
            ...
            for (var i = 0; i < TRX_ADDONS_STORAGE['menu_cache'].length; i++) {
                ...
                menu
                    .find('li')
                    .removeClass('current-menu-ancestor current-menu-parent current-menu-item current_page_item');
                menu
                    ...
                    .each(function (idx) {
                        ...
                        while ((li = li.parents('li')).length > 0) {
                            cnt++;
                            li.addClass('current-menu-ancestor' + (cnt == 1
                                ? ' current-menu-parent'
                                : ''));
                        }
                    });
        ...
        function trx_addons_comments_validate(form) {
            ...
            if (form.find('.comments_tieude input[aria-required="true"]').length > 0) {
                comments_args
                    .rules
                    .push({
                        field: 'tieude',
        ...
        container
            ...
            .each(function () {
                if (jQuery(this).hasClass('hover-animation-inited')) 
                    return;
                jQuery(this)
                    .addClass('hover-animation-inited')
                    .hover(function (e) {
                        jQuery(this)
                            .find('[data-hover-animation^="animated"]')
                            .each(function () {
    ...
    if (TRX_ADDONS_STORAGE['ajax_views']) {
        jQuery(document)
            .on('action.ready_trx_addons', function () {
                    ...
                    setTimeout(function () {
                        jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], {
                            ...
                            .done(function (response) {
                                ...
                                if (rez.error === '') {
                                    jQuery('.post_counters_single .post_counters_views .post_counters_number,.sc_layouts_tit' +
                                            'le_meta .post_counters_views .post_counters_number').html(rez.counter);
                                }
                            });
                    }, 10);
                }
    ...
    function trx_addons_scroll_actions() {
        ...
        jQuery('[data-animation^="animated"]:not(.animated)')
            .each(function () {
    ...
    function trx_addons_resize_video(e, cont) {
        ...
        cont
            ...
            .each(function () {
                ...
                var video = jQuery(this)
                    ...
                    .eq(0);
                var ratio = (video.data('ratio') !== undefined
                    ? video.data('ratio').split(':')
                    : [16, 9]);
                ratio = ratio.length != 2 || ratio[0] == 0 || ratio[1] == 0
                    ? 16 / 9
                    : ratio[0] / ratio[1];
                ...
                var w_real = Math.round(mejs_cont.length > 0
                        ? Math.min(percent
                            ? 10000
                            : w_attr, mejs_cont.parents('div,article').width())
                        : Math.min(percent
                            ? 10000
                            : w_attr, video.parents('div,article').width())),
                    h_real = Math.round(percent
                        ? w_real / ratio
                        : w_real / w_attr * h_attr);
        ...
        cont
            ...
            .each(function () {
                ...
                if (iframe.attr('src') === undefined || iframe.attr('src').indexOf('soundcloud') > 0) 
                    return;
                ...
                if (par.css('position') == 'absolute' && h_real > ph) {
                    h_real = ph;
                    w_real = Math.round(percent
                        ? h_real * ratio
                        : h_real * w_attr / h_attr)
                }
                    ...
                    if (col_height < row_height && scroll_offset + top_panel_fixed_height > row_top) {
                        ...
                        if (col_big) {
                            if (scroll_dir != TRX_ADDONS_STORAGE['scroll_dir_last'] && col.css('position') == 'fixed') {
                                ...
                                if (scroll_offset + jQuery(window).height() >= row_top + row_height + 30) {
                                    col_init.bottom = 0;
                                    col_init.position = 'absolute';
                                } else if (scroll_offset + jQuery(window).height() >= (col.css('position') == 'absolute'
                                    ? col_top
                                    : row_top) + col_height + 30) {
                                    col_init.bottom = 30;
                        ...
                        if (col_init.position != 'undefined') {
                            ...
                            if (!col.prev().hasClass('sc_column_fixed_placeholder')) {
                                ...
                                col.before('<div class="sc_column_fixed_placeholder ' + col.attr('class').replace('sc_column_fixed', '') + '"' + (col.data('col')
                                    ? ' data-col="' + col.data('col') + '"'
                                    : '') + '></div>');
                            }
                            col_init.left = col_init.position == 'fixed'
                                ? col
                                    .prev()
                                    .offset()
                                    .left
                            ...
                            col_init.width = col
                                .prev()
                                .width() + parseFloat(col.prev().css('paddingLeft')) + parseFloat(col.prev().css('paddingRight'));
        ...
        cont
            ...
            .each(function () {
                ...
                if (typeof params.type == 'undefined') 
                    params.type = obj.hasClass('vc_row') || obj.hasClass('wpb_column') || obj.hasClass('elementor-section') || obj.hasClass('elementor-column')
                        ? 'bg'
                        : 'object';
                ...
                if (typeof params.opacity == 'undefined') 
                    params.opacity = 0;
                var scroller = params.type == 'bg'
                    ? obj.find('.sc_parallax_scroller')
                    : obj;
                if (scroller.length == 0 && obj.css('background-image') != '') {
                    obj.prepend('<div class="sc_parallax_scroller" style="' + (obj.css('background') != ''
                        ? 'background:' + obj.css('background').replace(/"/g, '&quot;') + ';'
                        : '') + (obj.css('backgroundColor') != ''
                        ? 'background-color:' + obj.css('backgroundColor') + ';'
                        : '') + (obj.css('backgroundImage') != ''
                        ? 'background-image:' + obj.css('backgroundImage').replace(/"/g, '&quot;') + ';'
                        : '') + (obj.css('backgroundPosition') != ''
                        ? 'background-position:' + obj.css('backgroundPosition') + ';'
                        : '') + (obj.css('backgroundSize') != ''
                        ? 'background-size:' + obj.css('backgroundSize') + ';'
                        : '') + (obj.css('backgroundRepeat') != ''
                        ? 'background-repeat:' + obj.css('backgroundRepeat') + ';'
                        : '') + (obj.css('backgroundAttachment') != ''
                        ? 'background-attachment:' + obj.css('backgroundAttachment') + ';'
                        : '') + '"></div>');
                ...
                if (scroller.length > 0 && w_top <= obj_bottom && obj_top <= w_bottom) {
                    var delta = w_height + obj_height,
                        ...
                        step_y = params.y != 0
                            ? params.y / delta
                            : 0,
                        step_scale = params.scale != 0
                            ? params.scale / 100 / delta
                            : 0,
                        step_rotate = params.rotate != 0
                            ? params.rotate / delta
                            : 0,
                        step_opacity = params.opacity != 0
                            ? params.opacity / delta
                            : 0;
                    var scroller_init = {},
                        transform = '';
                    if (params.type == 'bg') {
                        scroller_init.width = obj_width + (params.x != 0
                            ? Math.abs(params.x)
                            : 0) + 'px';
                        scroller_init.height = obj_height + (params.y != 0
                            ? Math.abs(params.y)
                            : 0) + 'px';
                    ...
                    if (step_opacity != 0) {
                        scroller_init.opacity = trx_addons_round_number(start
                            ? 1 - shift * step_opacity + params.opacity
                            : 1 + shift * step_opacity, 2);
                    }
                    if (step_x != 0) {
                        transform += 'translateX(' + Math.round(start
                            ? params.x - shift * step_x
                            : shift * step_x - (params.type == 'bg' && params.x > 0
                                ? params.x
                                : 0)) + 'px)';
                    }
                    if (step_y != 0) {
                        transform += (transform != ''
                            ? ' '
                            : '') + 'translateY(' + Math.round(start
                            ? params.y - shift * step_y
                            : shift * step_y - (params.type == 'bg' && params.y > 0
                                ? params.y
                                : 0)) + 'px)';
                    }
                    if (step_rotate != 0) {
                        transform += (transform != ''
                            ? ' '
                            : '') + 'rotate(' + trx_addons_round_number(start
                            ? params.rotate - shift * step_rotate
                            : shift * step_rotate, 2) + 'deg)';
                    }
                    if (step_scale != 0) {
                        transform += (transform != ''
                            ? ' '
                            : '') + 'scale(' + trx_addons_round_number(start
                            ? 1 + params.scale / 100 - shift * step_scale
                            : 1 + shift * step_scale - (params.type == 'bg' && params.scale < 0
                                ? params.scale / 100
                                : 0), 2) + ')';
                    }
                    if (transform != '') {
...
jQuery(document).on('action.ready_trx_addons', function () {
    ...
    function trx_addons_login_validate(form) {
        form
            .find('input')
            .removeClass('trx_addons_field_error');
                ...
                .done(function (response) {
                    ...
                    if (rez.error === '') {
                        ...
                        result
                            ...
                            .html(TRX_ADDONS_STORAGE['msg_login_error'] + (rez.error !== undefined
                                ? '<br>' + rez.error
                                : ''));
                    }
                    result
            ...
            jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], {
                ...
                .done(function (response) {
                    ...
                    if (rez.error === '') {
                        ...
                        result
                            .addClass("trx_addons_message_box_error")
                            .html(TRX_ADDONS_STORAGE['msg_registration_error'] + (rez.error !== undefined
                                ? '<br>' + rez.error
                                : ''));
                    }
                    result
...
(function () {
    ...
    function trx_addons_sc_fullheight(e, container) {
        ...
        if (container === undefined || container.length === undefined || container.length == 0) 
            return;
        container
            .find('.trx_addons_stretch_height')
            .each(function () {
                    ...
                    if (!pagination_busy) {
                        ...
                        jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], {
                            ...
                            .done(function (response) {
                                    ...
                                    if (load_more) {
                                        if (posts.find('[class*="_tabs_list_item"]').length > 0) {
                                            posts
                                                ...
                                                .parent()
                                                .append(jQuery(rez.data).find('.sc_item_posts_container [class*="_tabs_list_item"]').parent().html());
                                            posts
                                                .find('[class$="_tabs_content"]')
                                                .append(jQuery(rez.data).find('.sc_item_posts_container [class$="_tabs_content"]').html());
                                            posts
                                                .find('[class*="_tabs_list_item_active"]')
                                                .each(function (idx) {
                                                    if (idx > 0) {
                                                        var classes = jQuery(this)
                                                                .attr('class')
                                                                .split(' '),
                                                            found = false;
                                                        for (var i = 0; i < classes.length; i++) {
                                            ...
                                            posts
                                                .find('[class$="_tabs_content"] [class*="_item_active"]')
                                                .each(function (idx) {
                                                    if (idx > 0) {
                                                        var classes = jQuery(this)
                                                                .attr('class')
                                ...
                                if (load_more) 
                                    pagination_wrap.removeClass('loading');
            ...
            if (jQuery('.sc_post_details_popup.inited').length > 0) {
                jQuery('body:not(.sc_post_details_popup_inited)')
                    ...
                    .on('click', '.trx_addons_post_details_popup_prev,.trx_addons_post_details_popup_next', function (e) {
                        ...
                        if (!post_item || post_item.length == 0) 
                            return;
            ...
            function trx_addons_show_post_details(post_item, show_overlay) {
                jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], {
                    ...
                    .done(function (response) {
                        ...
                        if (popup.length == 0) {
                            jQuery('body').append('<div id="trx_addons_post_details_popup_overlay"></div><div id="trx_addons_post_d' +
                                    'etails_popup"><div class="trx_addons_post_details_content"></div><span class="tr' +
                                    'x_addons_post_details_popup_close trx_addons_icon-cancel"></span><span class="tr' +
                                    'x_addons_post_details_popup_prev trx_addons_icon-left"></span><span class="trx_a' +
                                    'ddons_post_details_popup_next trx_addons_icon-right"></span></div>');
...
(function () {
    ...
    function trx_addons_js_composer_init(e, container) {
        if (container === undefined) 
            container = jQuery('body');
        if (container.length === undefined || container.length == 0) 
            return;
        container
...
jQuery(document).on('action.ready_trx_addons', function () {
    ...
    function trx_addons_cpt_layouts_fix_rows(rows, rows_always, resize) {
        ...
        rows.each(function () {
            ...
            var offset = parseInt(jQuery(this).hasClass('sc_layouts_row_fixed_on')
                ? placeholder.offset().top
                : jQuery(this).offset().top, 10);
            if (isNaN(offset)) 
                offset = 0;
...
jQuery(document).on('action.init_hidden_elements', function (e, container) {
    ...
    if (TRX_ADDONS_STORAGE['scroll_to_anchor'] == 1) {
        ...
        jQuery(window).bind('mousewheel DOMMouseScroll', function (e) {
            ...
            var wheel_dir = e.originalEvent.wheelDelta > 0 || e.originalEvent.detail < 0
                ? -1
                : 1;
            ...
            var ah = jQuery('#wpadminbar').length > 0
    ...
    function trx_addons_detect_active_toc() {
        ...
        toc_menu_items.each(function (idx) {
            ...
            var pos = id.indexOf('#');
            if (pos < 0 || id.length == 1) 
                return;
            var href = jQuery(this)
                .find('a')
                .attr('href');
            if (!trx_addons_is_local_link(href) || jQuery(id).length == 0) 
                return;
            var off = jQuery(id)
                .offset()
                .top;
        ...
        if (pos >= 0 && href.length > 1 && trx_addons_is_local_link(href)) {
            ...
            trx_addons_document_animate_to(href.substr(pos), function () {
                if (TRX_ADDONS_STORAGE['update_location_from_anchor'] == 1) {
                    ...
                    if (loc_pos > 0) 
                        loc = loc.substring(0, loc_pos);
                    trx_addons_document_set_location(pos == 0
                        ? loc + href
                        : href);
                }
            });
    ...
    function trx_addons_build_page_toc() {
        ...
        jQuery('[id^="toc_menu_"],.sc_anchor').each(function (idx) {
            ...
            if (row.length == 0) 
                row = obj.parent();
            var row_id = row.length > 0 && row.attr('id') !== undefined && row.attr('id') !== ''
                ? row.attr('id')
                : '';
            ...
            toc_count++;
            toc += '<div class="toc_menu_item' + (separator == 'yes'
                ? ' toc_menu_separator'
                : '') + '" data-id="' + id + '">' + (title || description
                ? '<a href="' + (url
                    ? url
                    : '#' + id) + '" class="toc_menu_description">' + (title
                    ? '<span class="toc_menu_description_title">' + title + '</span>'
                    : '') + (description
                    ? '<span class="toc_menu_description_text">' + description + '</span>'
                    : '') + '</a>'
                : '') + '<a href="' + (url
                ? url
                : '#' + id) + '" class="toc_menu_icon ' + icon + '"' + (sow_icon
                ? ' data-sow-icon="' + sow_icon + '"'
                : '') + '></a></div>';
        });
        if (toc_count > 0) 
...
jQuery(document).on('action.init_hidden_elements', function (e, container) {
    ...
    jQuery('input[type="text"]:not(.fill_inited),input[type="number"]:not(.fill_inited),inpu' +
                't[type="search"]:not(.fill_inited),input[type="password"]:not(.fill_inited),inpu' +
                't[type="email"]:not(.fill_inited),textarea:not(.fill_inited),select:not(.fill_in' +
                'ited)')
        .each(function () {
            var fld = jQuery(this);
            sc_form_mark_filled(fld);
            fld
                .addClass('fill_inited')
    ...
    function sc_form_validate(form) {
        ...
        form
            .find('input')
            .removeClass('trx_addons_error_field');
        var error = trx_addons_form_validate(form, {
            rules: [
                {
                    field: "name",
...
(function () {
    ...
    jQuery(document).on('action.init_hidden_elements', function (e, container) {
        ...
        if (gmap.length > 0) {
            ...
            }, gmap.parents('.elementor-element-editable').length > 0
                ? 500
                : 0);
        }
    });
    ...
    function trx_addons_sc_googlemap_create(map, coords) {
        ...
        googlemap.maps[id] = {
            ...
            opt: {
                ...
                styles: TRX_ADDONS_STORAGE['googlemap_styles'][coords.style
                        ? coords.style
                        : 'default'],
    ...
    function trx_addons_sc_googlemap_build(id) {
        googlemap.maps[id].map = new google
            .maps
            .Map(googlemap.maps[id].dom, googlemap.maps[id].opt);
        googlemap.maps[id].fit_to_bounds = googlemap.maps[id].opt['center'] || googlemap.maps[id].markers.length > 1;
        googlemap.maps[id].bounds = new google
            .maps
            .LatLngBounds();
        if (googlemap.maps[id].opt['center'] && googlemap.maps[id].markers.length == 1) {
            googlemap
                .maps[id]
                .bounds
                .extend(googlemap.maps[id].opt['center']);
        }
            ...
            if (googlemap.maps[id].map) {
                if (googlemap.maps[id].opt['center']) {
                    googlemap
                        .maps[id]
                        .map
                        .setCenter(googlemap.maps[id].opt['center']);
                }
                if (googlemap.maps[id].fit_to_bounds) {
                    googlemap
                        .maps[id]
                        .map
                        .fitBounds(googlemap.maps[id].bounds);
                    googlemap
                        .maps[id]
                        .map
                        .panToBounds(googlemap.maps[id].bounds);
                }
            }
        });
    }
    function trx_addons_sc_googlemap_add_markers(id) {
        ...
        for (var i = 0; i < googlemap.maps[id].markers.length; i++) {
            ...
            if (googlemap.maps[id].markers[i].latlng == '') {
                if (googlemap.maps[id].geocoder_request !== false) 
                    continue;
                        ...
                        if (status == google.maps.GeocoderStatus.OK) {
                            try {
                                ...
                                if (results[0].geometry.location.lat && results[0].geometry.location.lng) 
                                    ...
                                    googlemap.maps[id].markers[idx].latlng = results[0]
                                        .geometry
                                        .location
                                        .toString()
                                        .replace(/\(\)/g, '');
                ...
                var markerInit = {
                    ...
                    position: new google
                        .maps
                        .LatLng(latlngStr[0], latlngStr[1]),
                ...
                if (googlemap.maps[id].markers[i].icon) {
                    markerInit.icon = googlemap.maps[id].markers[i].icon_width > 0 && googlemap.maps[id].markers[i].icon_height > 0
                        ? new google
                            .maps
                            .MarkerImage(googlemap.maps[id].markers[i].icon, null, null, null, new google.maps.Size(googlemap.maps[id].markers[i].icon_width, googlemap.maps[id].markers[i].icon_height))
                ...
                googlemap.maps[id].markers[i].marker = new google
                    .maps
                    .Marker(markerInit);
                ...
                if (googlemap.maps[id].markers[i].description !== '') {
                    googlemap.maps[id].markers[i].infowindow = new google
                        .maps
                        .InfoWindow({
                    ...
                    google
                        .maps
                        .event
                        .addListener(googlemap.maps[id].markers[i].marker, "click", function (e) {
                            var latlng = e
                                .latLng
                                .toString()
                                .replace("(", '')
                            ...
                            for (var j = 0; j < googlemap.maps[id].markers.length; j++) {
                                if (trx_addons_googlemap_compare_latlng(latlng, googlemap.maps[id].markers[j].latlng)) {
                                    ...
                                    googlemap.maps[id].old_zoom = googlemap
                                        .maps[id]
                                        .map
                                        .getZoom();
                                    googlemap
                                        .maps[id]
                                        .map
                                        .setZoom(Math.max(16, googlemap.maps[id].old_zoom + 2));
                                    googlemap.maps[id].old_center = googlemap
                                        .maps[id]
                                        .map
                                        .getCenter();
                                    ...
                                    googlemap
                                        .maps[id]
                                        .map
                                        .setCenter(new google.maps.LatLng(center[0], center[1]));
                                    ...
                                    googlemap
                                        ...
                                        .markers[j]
                                        .infowindow
                                        .open(googlemap.maps[id].map, googlemap.maps[id].markers[j].marker);
                                    break;
                                }
                            }
                        });
                    google
                        .maps
                        .event
                        .addListener(googlemap.maps[id].markers[i].infowindow, 'closeclick', function () {
                            googlemap
                                .maps[id]
                                .map
                                .setZoom(googlemap.maps[id].old_zoom);
                            googlemap
                                .maps[id]
                                .map
                                .setCenter(googlemap.maps[id].old_center);
                            if (googlemap.maps[id].opened_marker.animation) {
                                googlemap
                                    .maps[id]
                                    .opened_marker
                                    .marker
                                    .setAnimation(googlemap.maps[id].opened_marker.animation);
                            }
                        });
                }
...
function MarkerClusterer(a, b, c) {
    ...
    navigator
        .userAgent
        .toLowerCase()
        .indexOf("msie") !== -1 && (this.batchSize_ = this.batchSizeIE_),
    this.setupStyles_(),
    this.addMarkers(b, !0),
    this.setMap(a)
}
ClusterIcon.prototype.onAdd = function () {
    ...
    this.boundsChangedListener_ = google
        .maps
        .event
        .addListener(this.getMap(), "bounds_changed", function () {
            c = b
        }),
    google
        .maps
        .event
        .addDomListener(this.div_, "mousedown", function () {
            b = !0,
            c = !1
        }),
    google
        .maps
        .event
        .addDomListener(this.div_, "click", function (d) {
            if (b = !1, !c) {
                var e,
                    f,
                    g = a
                ...
                google
                    .maps
                    .event
                    .trigger(g, "click", a.cluster_),
                google
                    .maps
                    .event
                    .trigger(g, "clusterclick", a.cluster_),
                g.getZoomOnClick() && (f = g.getMaxZoom(), e = a.cluster_.getBounds(), g.getMap().fitBounds(e), setTimeout(function () {
                    g
                        .getMap()
                        .fitBounds(e),
                    null !== f && g
    ...
    google
        .maps
        .event
        .addDomListener(this.div_, "mouseover", function () {
            var b = a
                .cluster_
                .getMarkerClusterer();
            google
                .maps
                .event
                .trigger(b, "mouseover", a.cluster_)
        }),
    google
        .maps
        .event
        .addDomListener(this.div_, "mouseout", function () {
            var b = a
                .cluster_
                .getMarkerClusterer();
            google
...
ClusterIcon.prototype.show = function () {
    if (this.div_) {
        ...
        this
            .cluster_
            .getMarkerClusterer()
            .enableRetinaIcons_ || (a += "clip: rect(" + -1 * d + "px, " + (-1 * c + this.width_) + "px, " + (-1 * d + this.height_) + "px, " + -1 * c + "px);"),
        a += "'>",
...
Cluster.prototype.getBounds = function () {
    var a,
        b = new google
            .maps
            .LatLngBounds(this.center_, this.center_),
        c = this.getMarkers();
...
Cluster.prototype.updateIcon_ = function () {
    ...
    var c = this
        ...
        d = this
            .markerClusterer_
            .getCalculator()(this.markers_, c);
    this
        .clusterIcon_
        .setCenter(this.center_),
    this
        .clusterIcon_
        .useStyle(d),
    this
...
MarkerClusterer.prototype.onAdd = function () {
    ...
    this.listeners_ = [
        google
            .maps
            .event
            .addListener(this.getMap(), "zoom_changed", function () {
                a.resetViewport_(!1),
                this.getZoom() !== (this.get("minZoom") || 0) && this.getZoom() !== this.get("maxZoom") || google
                    .maps
                    .event
                    .trigger(this, "idle")
            }),
        google
            .maps
            .event
            .addListener(this.getMap(), "idle", function () {
                a.redraw_()
            })
    ]
},
...
MarkerClusterer.prototype.pushMarkerTo_ = function (a) {
    if (a.getDraggable()) {
        ...
        google
            .maps
            .event
            .addListener(a, "dragend", function () {
...
MarkerClusterer.prototype.getExtendedBounds = function (a) {
    var b = this.getProjection(),
        c = new google
            .maps
            .LatLng(a.getNorthEast().lat(), a.getNorthEast().lng()),
        d = new google
            .maps
            .LatLng(a.getSouthWest().lat(), a.getSouthWest().lng()),
...
MarkerClusterer.prototype.addToClosestCluster_ = function (a) {
    ...
    for (b = 0; b < this.clusters_.length; b++) 
        d = this.clusters_[b],
        e = d.getCenter(),
        e && (c = this.distanceBetweenPoints_(e, a.getPosition()), c < f && (f = c, g = d));
...
MarkerClusterer.prototype.createClusters_ = function (a) {
    ...
    if (this.ready_) {
        ...
        d = this
            ...
            ? new google
                .maps
                .LatLngBounds(this.getMap().getBounds().getSouthWest(), this.getMap().getBounds().getNorthEast())
            : new google
                .maps
                .LatLngBounds(new google.maps.LatLng(85.02070771743472, -178.48388434375), new google.maps.LatLng(-85.08136444384544, 178.00048865625));
...
MarkerClusterer.IMAGE_PATH = "http://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerclustererpl" +
        "us/images/m",
...
(function (window, document) {
    ...
    Pathformer.prototype.parseAttr = function (element) {
        ...
        for (var i = 0; i < element.length; i++) {
            ...
            if (this.ATTR_WATCH.indexOf(attr.name) !== -1 && attr.value.indexOf('%') !== -1) {
                throw new Error('Pathformer [parseAttr]: a SVG shape got values in percentage. This cannot be tra' +
                        'nsformed into \'path\' tags. Please use \'viewBox\'.');
            }
        ...
        switch (element.constructor) {
            ...
            case window.HTMLObjectElement:
                ...
                onLoad = function (e) {
                    ...
                    self.el = element.contentDocument && element
                        .contentDocument
                        .querySelector('svg');
                    if (!self.el && e) {
                ...
                break;
            default:
                throw new Error('Vivus [constructor]: "element" parameter is not valid (or miss the "file" attrib' +
                        'ute)');
        }
    };
    ...
    Vivus.prototype.mapping = function () {
        ...
        this.delay = this.delay === null
            ? this.duration / 3
            : this.delay;
        this.delayUnit = this.delay / (paths.length > 1
            ? paths.length - 1
            : 1);
        for (i = 0; i < this.map.length; i++) {
            ...
            switch (this.type) {
                ...
                case 'scenario-sync':
                    ...
                    timePoint = pAttrs['data-async'] !== undefined
                        ? pathObj.startAt
    ...
    Vivus.prototype.renderPath = function (index) {
        if (this.forceRender && this.map && this.map[index]) {
            ...
            pathObj
                .el
                .parentNode
                .replaceChild(newPath, pathObj.el);
            pathObj.el = newPath;
        }
    };
...
jQuery(document).on('action.init_hidden_elements', function (e, container) {
    ...
    var time = 50;
    container
        .find('.sc_icon_type_svg.sc_icon_animation:not(.inited)')
        .each(function (idx) {
...
(function () {
    ...
    function trx_addons_sc_skills_init(e, container) {
        ...
        container
            .find('.sc_skills_item:not(.inited)')
            .each(function () {
                var skillsItem = jQuery(this);
                if (jQuery(this).parents('div:hidden,article:hidden').length > 0) {
                    return;
                }
                var scrollSkills = skillsItem
                    .offset()
                    .top;
                if (scrollPosition > scrollSkills) {
                    var init_ok = true;
                    var skills = skillsItem
                        .parents('.sc_skills')
                        .eq(0);
                    ...
                    if (type == 'bar') {
                        ...
                        var count = skillsItem
                            .find('.sc_skills_count')
                            .eq(0);
                        if (dir == 'horizontal') 
                        ...
                        if (window.Chart) {
                            ...
                            var options = {
                                ...
                                animation: skillsItem
                                    .parents('.vc_row[data-vc-full-width="true"]')
                                    .length == 0,
                                animationSteps: steps,
                                animationEasing: easing,
                                animateRotate: true,
                                animateScale: skillsItem
                                    .parents('.vc_row[data-vc-full-width="true"]')
                                    .length == 0
                            };
                            var pieData = [];
        ...
        jQuery('.sc_skills_pie canvas')
            .each(function () {
                ...
                if (skillsItem.width() != canvas.width()) {
                    ...
                    if (data === undefined || opt === undefined) 
                        return;
                    canvas
                        .empty()
                        .attr({
...
jQuery(window).load(function () {
    "use strict";
    jQuery('.trx_addons_audio_wrap:not(.inited)')
        .addClass('inited')
        .each(function () {
            ...
            if (audio_wrap.hasClass('list')) {
                ...
                audio_wrap
                    .find('.trx_addons_audio_navigation')
                    .on('click', '.nav_btn', function (e) {
                        ...
                        if (jQuery(this).hasClass('prev')) {
                            if (current.is(':first-child')) {
                                current = audio_wrap
                                    .find('.trx_addons_audio_player:last-child')
                                    .addClass('current')
                                    .show();
                            } else {
                                current = current
                        ...
                        if (jQuery(this).hasClass('next')) {
                            if (current.is(':last-child')) {
                                current = audio_wrap
                                    .find('.trx_addons_audio_player:first-child')
                                    .addClass('current')
                                    .show();
                            } else {
                                current = current
...
function trx_addons_audio_height() {
    ...
    jQuery('.trx_addons_audio_wrap')
        .each(function () {
            jQuery(this).removeClass('resized');
            if (jQuery(this).hasClass('list')) {
                var height = 0;
                jQuery(this)
                    .find('.trx_addons_audio_player')
                    .each(function () {
...
(function () {
    ...
    function trx_addons_init_sliders(e, container) {
        if (container.find('.sc_slider_controller:not(.inited)').length > 0) {
            container
                .find('.sc_slider_controller:not(.inited)')
                .each(function () {
                    ...
                    jQuery('#' + slider_id + ' .swiper-slide').each(function (idx) {
                        ...
                        controller_html += '<div class="slider-slide swiper-slide" style="' + (image !== undefined
                            ? 'background-image: url(' + image + ');'
                            : '') + '"><div class="sc_slider_controller_info"><span class="sc_slider_controller_info_' +
                                'number">' + (idx < 9
                            ? '0'
                            : '') + (idx + 1) + '</span><span class="sc_slider_controller_info_title">' + title + '</span></div></div>';
                    });
                    controller.html('<div id="' + controller_id + '_outer" class="slider_outer slider_swiper_outer slider_style_controller slider_o' +
                            'uter_' + (controller_controls == 1
                        ? 'controls slider_outer_controls_side'
                        : 'nocontrols') + ' slider_outer_nopagination slider_outer_' + (controller_per_view == 1
                        ? 'one'
                        : 'multi') + ' slider_outer_direction_' + (controller_direction == 'vertical'
                        ? 'vertical'
                        : 'horizontal') + '"><div id="' + controller_id + '_swiper" class="slider_container slider_swiper swiper-slider-container slider_' + (controller_controls == 1
                        ? 'controls slider_controls_side'
                        : 'nocontrols') + ' slider_nopagination slider_notitles slider_noresize slider_' + (controller_per_view == 1
                        ? 'one'
                        : 'multi') + ' slider_direction_' + (controller_direction == 'vertical'
                        ? 'vertical'
                        : 'horizontal') + '" data-slides-min-width="100" data-controlled-slider="' + slider_id + '" data-direction="' + (controller_direction == 'vertical'
                        ? 'vertical'
                        : 'horizontal') + '"' + (controller_effect !== undefined
                        ...
                        ? '<div class="slider_controls_wrap"><a class="slider_prev swiper-button-prev" href' +
                            '="#"></a><a class="slider_next swiper-button-next" href="#"></a></div>'
                        : '') + '</div>');
                });
        }
        if (container.find('.sc_slider_controls:not(.inited)').length > 0) {
            container
                .find('.sc_slider_controls:not(.inited)')
                .each(function () {
                    ...
                    if (pagination_style == 'thumbs') {
                        ...
                        slides.each(function () {
                            var slide = jQuery(this);
                            var image = slide.data('image');
                            html += '<span class="slider_pagination_button_wrap swiper-pagination-button-wrap" style=' +
                                    '"width: ' + Math.round(100 / slides.length, 2) + '%;"><span class="slider_pagination_button swiper-pagination-button"' + (image !== undefined
                        ...
                        controls
                            .find('.slider_pagination_wrap')
                            .html('<span class="slider_pagination_current swiper-pagination-current">1</span>/<span' +
                                    ' class="slider_pagination_total swiper-pagination-total">' + jQuery('#' + slider_id + ' .swiper-slide').length + '</span>');
                    } else if (pagination_style == 'bullets') {
                        var html = '';
                        jQuery('#' + slider_id + ' .swiper-slide').each(function () {
                            html += '<span class="slider_pagination_bullet swiper-pagination-bullet" data-slide-numbe' +
                                    'r="' + jQuery(this).data('slide-number') + '"></span>';
                        });
                        controls
                            .find('.slider_pagination_wrap')
                            .html(html);
                    ...
                    if (pagination_style != 'none') {
                        ...
                        jQuery('#' + slider_id)
                            .on('slider_init slide_change_start', function (e) {
                                if (TRX_ADDONS_STORAGE['swipers'][slider_id]) {
                                    ...
                                    if (total > 0) {
                                        if (pagination_style == 'progress') {
                                            ...
                                            controls
                                                .find('.slider_pagination_current')
                                                .text(current);
                                        }
                                    }
                                }
        ...
        if (container.find('.slider_swiper:not(.inited)').length > 0) {
            container
                ...
                .each(function () {
                    ...
                    var cont = slider
                        .parent()
                        .hasClass('slider_swiper_outer')
                        ? slider
                            .parent()
                            .attr('id', id + '_outer')
                        : slider;
                    var cont_id = cont.attr('id');
                    var is_controller = slider
                        .parents('.sc_slider_controller')
                        .length > 0;
                    ...
                    if (direction == 'vertical' && slider.hasClass('slider_height_auto') && slider.hasClass('slider_noresize')) {
                        ...
                        slider
                            ...
                            .each(function (idx) {
                                if (idx >= spv) 
                                    return;
                                height += jQuery(this).height() + (idx > 0
                                    ? space
                                    : 0);
                            });
                        if (height > 0) 
                    ...
                    if (isNaN(interval)) 
                        interval = 0;
                    var noswipe = slider.hasClass('slider_noswipe') || slider
                        .parents('.slider_noswipe,.elementor-edit-mode')
                        .length > 0;
                    ...
                    TRX_ADDONS_STORAGE['swipers'][id] = new Swiper('.' + id, {
                        ...
                        pagination: {
                            ...
                            clickable: slider.hasClass('slider_pagination')
                                ? '#' + cont_id + ' .slider_pagination_wrap'
                                : false,
                            type: slider.hasClass('slider_pagination') && slider.data('pagination')
                        ...
                        autoplay: slider.hasClass('slider_noautoplay') || (typeof slider.data('autoplay') !== 'undefined' && slider.data('autoplay') == 0) || interval == 0
                            ? false
                            : {
                        ...
                        on: {
                            slideChangeTransitionStart: function () {
                                ...
                                cont
                                    ...
                                    .removeClass('active')
                                    .fadeOut();
                                var controlled_slider = jQuery('#' + slider.data(is_controller
                                    ? 'controlled-slider'
                                    : 'controller') + ' .slider_swiper');
                    ...
                    cont
                        ...
                        .on('click', '>span', function (e) {
                            ...
                            var t = jQuery(this)
                                .addClass('swiper-pagination-button-active')
                                .index() * TRX_ADDONS_STORAGE['swipers'][id].params.slidesPerGroup;
        ...
        if (container.find('.slider_elastistack:not(.inited)').length > 0) {
            container
                .find('.slider_elastistack:not(.inited)')
                .each(function () {
                    if (jQuery(this).parents('div:hidden,article:hidden').length > 0) 
                        return;
                    var slider = jQuery(this);
                ...
                if (last_width == 0 || last_width != slider_width) {
                    ...
                    if (!slider.hasClass('slider_noresize') || slider.height() == 0) {
                        ...
                        if (ratio === undefined || ('' + ratio).indexOf(':') < 1) {
                            ratio = slide_height > 0
                                ? slide_width + ':' + slide_height
                                : "16:9";
                            slider.attr('data-ratio', ratio);
                        }
                        ratio = ratio.split(':');
                        var ratio_x = !isNaN(ratio[0])
                            ? Number(ratio[0])
                            : 16;
                        var ratio_y = !isNaN(ratio[1])
                            ? Number(ratio[1])
                            : 9;
                        var height = Math.floor((spv == 1
                            ? slider_width
                            : slide_width) / ratio_x * ratio_y);
                        slider.height(direction == 'vertical'
                            ? height * spv + (spv - 1) * space
                            : height);
                        on_resize = true;
    ...
    function trx_addons_set_controller_height(controller_id, slider) {
        ...
        if (controller && controller.length > 0 && controller.hasClass('sc_slider_controller_vertical') && controller.hasClass('sc_slider_controller_height_auto')) {
            if (jQuery(window).width() >= 768) {
                ...
                if (isNaN(controller_spv)) 
                    controller_spv = 1;
                controller
                    .find('.slider_container')
                    .height(controller_spv * 100);
            }
        }
    }
})();
    ...
    if (jQuery('.sc_layouts_cart').length > 0 && !jQuery('body').hasClass('added_to_cart_inited')) {
        ...
        jQuery(document).on('added_to_cart removed_from_cart', function () {
            ...
            var items = jQuery('.sc_layouts_cart_items')
                ...
                .split(' ', 2);
            items[0] = cnt;
            jQuery('.sc_layouts_cart_items').text(items[0] + (items.length > 1
                ? ' ' + items[1]
                : ''));
...
(function () {
    ...
    jQuery(document).on('action.before_ready_trx_addons', function () {
                ...
                .each(function () {
                    ...
                    jQuery(this)
                        ...
                        .superfish({
                            ...
                            speed: animation_in != 'none'
                                ? 500
                                : 200,
                            speedOut: animation_out != 'none'
                                ? 500
                                : 200,
                            autoArrows: false,
                            dropShadows: false,
                            onBeforeShow: function (ul) {
                                if (jQuery(this).parents("ul").length > 1) {
                                    var w = jQuery('.page_wrap').length > 0
                                ...
                                if (jQuery(this).parents('.top_panel').length > 0) {
                                    ...
                                    if (row_offset + par_offset + ul_height > w_height) {
                                        if (par_offset > ul_height) {
                                            jQuery(this).css({'top': 'auto', 'bottom': '-1.4em'});
                                        } else {
                                            jQuery(this).css({
        ...
        jQuery('.menu_hover_slide_line:not(.slide_inited),.menu_hover_slide_box:not(.slide_inite' +
                    'd)').each(function () {
            var menu = jQuery(this).addClass('slide_inited');
            var style = menu.hasClass('menu_hover_slide_line')
                ? 'line'
                : 'box';
            setTimeout(function () {
                if (jQuery.fn.spasticNav !== undefined) {
                    menu
                        .find('>ul')
                        .spasticNav({style: style, colorOverride: false});
                }
            }, 500);
        });
    ...
    function trx_addons_menu_collapse() {
        if (TRX_ADDONS_STORAGE['menu_collapse'] == 0) 
            return;
        jQuery('.sc_layouts_menu:not(.sc_layouts_menu_dir_vertical)')
            .each(function () {
                if (jQuery(this).parents('div:hidden,section:hidden,article:hidden').length > 0) 
                    return;
                ...
                if (!sc_layouts_item.parent().hasClass('wpb_wrapper') && !sc_layouts_item.parent().hasClass('sc_layouts_column') && !sc_layouts_item.parent().hasClass('elementor-widget-wrap')) 
                    return;
                var w_max = sc_layouts_item
                    .parent()
                    .width() - (Math.ceil(parseFloat(sc_layouts_item.css('marginLeft'))) + Math.ceil(parseFloat(sc_layouts_item.css('marginRight')))) - 2;
                var w_siblings = 0,
                    in_group = 0,
                    ul_id = ul.attr('id');
                sc_layouts_item
    ...
    $.fn.spasticNav = function (options) {
        ...
        return this.each(function () {
            ...
            if (isNaN(pl)) 
                pl = 0;
            $('<li id="blob"></li>').css({
                width: options.style == 'box'
                    ? a.outerWidth()
                    : a.width(),
                left: currentPageItem
                    .position()
                    .left + (options.style == 'box'
                    ? 0
                    : pl),
                top: currentPageItem
                    .position()
                    .top - options.overlap / 2,
                opacity: hidden
                    ? 0
                    : 1
            }).appendTo(this);
            ...
            if (options.colorOverride) {
                ...
                blob.css({
                    backgroundColor: hidden || bg == 'transparent'
                        ? options.color
                        : bg
                });
            }
...
jQuery(document).on('action.ready_trx_addons', function () {
    "use strict";
    jQuery('body:not(.woocommerce) .widget_area:not(.footer_wrap) .widget_product_categories' +
                ':not(.inited)').each(function () {
        var widget = jQuery(this).addClass('inited');
        widget
            .find('ul.product-categories .has_children > a')
                ...
                if (sb.width() == sb.parents('.content_wrap').width()) {
                    ...
                    if (!cat_menu.hasClass('inited')) {
                        ...
                        cat_menu
                            ...
                            .addClass('animated');
                        trx_addons_init_sfmenu('body:not(.woocommerce) .widget_area:not(.footer_wrap) .widget_product_categories' +
    ...
    jQuery('.variations_form.cart:not(.inited)').each(function () {
        ...
        if (trx_addons_attribs.length == 0) 
            return;
        trx_addons_attribs
            .on('click', function (e) {
    ...
    function trx_addons_woocommerce_check_variations(form, exclude) {
        setTimeout(function () {
            ...
            form
                ...
                .each(function () {
                    var select_box = jQuery(this);
                    var attrib_box = select_box
                        .siblings('.trx_addons_attrib_extended')
                        .length == 1
                        ? select_box.siblings('.trx_addons_attrib_extended')
                        : select_box
                            .parent()
                            .siblings('.trx_addons_attrib_extended');
                    if (select_box.attr('id') != exclude) 
                        select_box.trigger('touchstart');
                    attrib_box
    ...
    jQuery('.trx_addons_woocommerce_search_form:not(.inited)')
        .each(function () {
            jQuery(this)
                .addClass('inited')
                ...
                .on('click', '.trx_addons_woocommerce_search_form_field_label', function (e) {
                    jQuery(this)
                        .siblings('.trx_addons_woocommerce_search_form_field_list')
                        .slideToggle();
                })
                .on('click', '.trx_addons_woocommerce_search_form_field_list li', function (e) {
                    var list = jQuery(this).parent();
                    list
                        .siblings('.trx_addons_woocommerce_search_form_field_label')
